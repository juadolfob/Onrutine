...
export const insert = new ValidatedMethod({
  ...
  validate: Todos.simpleSchema()
    .pick(['listId', 'text', 'dueDate'])
    .validator({ clean: true, filter: false }),
  run({ listId, text, dueDate }) {
...
export const updateDueDate = new ValidatedMethod({
  name: 'todos.updateDueDate',
  validate: new SimpleSchema({
    todoId: Todos.simpleSchema()
      .schema('_id'),
    nextDueDate: Todos.simpleSchema()
      .schema('dueDate'),
  }).validator({ clean: true, filter: false }),
  run({ todoId, nextDueDate }) {
    // This is complex auth stuff - perhaps denormalizing a userId onto todos
    // would be correct here?
    const todo = Todos.findOne(todoId);
    ...
    Todos.update(todoId, {
      $set: {
        dueDate: (_.isUndefined(nextDueDate) ? null : nextDueDate),
      },
    });
  },
});
